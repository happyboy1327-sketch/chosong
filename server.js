...
app.get("/api/search", async (req, res) => {
  ...
  zipfile.on("entry", entry => {
    // entry.fileName은 이제 문자열
    const filename = typeof entry.fileName === "string" ? entry.fileName : entry.fileName.toString("utf8");
          ...
          if (Array.isArray(items)) {
            for (const raw of items) {
              ...
              if (wordRaw.toLowerCase().includes(q.toLowerCase())) {
                ...
                results.push({ word: wordRaw, hint: hint || "정의 없음" });
              }
            }
          }
        } catch (e) {
  ...
  // 안전장치: 일정 시간 후에도 응답이 없으면 결과 반환
  setTimeout(() => {
    if (!responded) {
      responded = true;
      console.warn("zip processing timeout, returning partial results");
      return res.json(results);
    }
  }, 5000); // 필요시 시간 조정
});
